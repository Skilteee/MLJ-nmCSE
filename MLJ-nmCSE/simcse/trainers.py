...
# from SentEval.senteval import engine as engine
...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        metrics = {"eval_sts12_spearman": sts12_spearman}
        if eval_senteval_transfer or self.args.eval_transfer:
            avg_transfer = 0
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if ((step + 1) % self.args.gradient_accumulation_steps != 0) and self.args.local_rank != -1:
                    ...
                    # print(global_step)
                    inputs['epochs'] = global_step
                    inputs['save_path'] = self.args.save_path
                    inputs['max_fill'] = self.args.max_fill
                    inputs['alpha'] = self.args.alpha
                    global_step += 1
                    tr_loss += self.training_step(model, inputs)
                self._total_flos += self.floating_point_ops(inputs)

                if (global_step - 1) % (self.args.eval_steps + 1) == 0:
                    logger.info("***** Eval results *****")
                    results = self.evaluate(eval_senteval_transfer=False)
                    # for key, value in sorted(results.items()):
                    logger.info(f" eval_sts12_spearman = {results['eval_sts12_spearman']}")
