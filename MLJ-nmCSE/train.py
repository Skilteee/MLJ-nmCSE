...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default='./bert', # xlm-roberta-base
        # default = './result/my-unsup-simcse-bert-base-uncased/checkpoint-3500',
        metadata={
...
class OurTrainingArguments(TrainingArguments):
    ...
    output_dir: str = field(
        default='result/my-unsup-simcse-bert-base-uncased',
    )
    do_train: bool = field(
        default=True
    )
    ...
    save_path: str = field(
        default= time.strftime("%m-%d-%H-%M-%S",time.localtime())
    )
    max_fill: float = field(
        default = 0.8
    )
    alpha: float = field(
        default = 7e-3
...
def main():
    ...
    if extension == "csv":
        ...
        datasets = load_dataset(extension, data_files=data_args.train_file)
    ...
    if training_args.do_train:
        ...
        if not os.path.exists('./result/run/{}'.format(training_args.save_path)):
            os.makedirs('./result/run/{}'.format(training_args.save_path))
        f = open('./result/run/{}/training_args.txt'.format(training_args.save_path), 'w', encoding='utf-8')
    ...
    if training_args.do_eval:
        ...
        output_eval_file = os.path.join('./result/run/{}'.format(training_args.save_path), "train_results.txt")
        if trainer.is_world_process_zero():
